"""Jump Shot Gym — Sync script (Acuity scheduling, no auth)"""
import os,csv,sys,datetime,argparse,requests,re,time

SYNC_DAYS=60
CC_USER=os.environ.get("CC_JUMP_SHOT_USER","localloginformaint.e.n.anc.e.1.99.40@gmail.com")
CC_PASS=os.environ.get("CC_JUMP_SHOT_PASS","H#r6v8npPgpuqXayO6x*nqKQJDU524")
...
def fetch_resource(resource_type,start,end):
    rows=[]; cur=new Date=start
    while cur<=end:
        payload={"MIME Type":"application/x-www-form-urlencoded; charset=UTF-8","type":resource_type,
                 "calendar":"9796667","month":cur.strftime("%Y-%m-%d"),"timezone":"America/New_York",
                 "skip":"true","options[numDays]":5,"ignoreAppointment":"","appointmentType":"category:Reservations","calendarID":""}
        h={"User-Agent":"Mozilla/5.0"}
        try:
            r=requests.post("https://jumpshotgym.as.me/schedule.php?action=showCalendar&fulldate=1&owner=31676056&template=weekly",
                data=payload,headers=h,timeout=30)
            matches=re.findall(r'value="(.{16})',r.text)
            for m in matches:
                try:
                    dt=datetime.datetime.strptime(m,"%Y-%m-%d %H:%M")
                    end_dt=dt+datetime.timedelta(minutes=45)
                    rows.append({"Date":dt.strftime("%m/%d/%Y"),"Start":dt.strftime("%I:%M:%S %p").lstrip("0"),
                                 "End":end_dt.strftime("%I:%M:%S %p").lstrip("0"),"Resource":RESOURCES[resource_type]})
                except: pass
        except Exception as e:
            print(f"      {resource_type} {cur}: {e}")
        time.sleep(2); cur+=datetime.timedelta(days=5)
    return rows
...
def collect(start,end):
    ...
    for rt,name in RESOURCES.items():
        rows=fetch_resource(rt,start,end); all_rows.extend(rows)
    ...
    print(f"      Total: {len(unique)} rows"); return unique

def save_csv(rows,path=None):
    if not path:
        ts=datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        path=os.path.join(os.path.dirname(os.path.abspath(__file__)),f"jump_shot_gym_{ts}.csv")
