...
SYNC_DAYS=60
CC_USER=os.environ.get("CC_TJS_USER",""); CC_PASS=os.environ.get("CC_TJS_PASS",""); CC_FAC=os.environ.get("CC_TJS_FAC","0")
...
def fetch(start,end,token):
    ...
    e=int(datetime.datetime.combine(end,datetime.time(23,59,59)).timestamp())
    rows=[]; page=1; has_more=True
    while has_more:
        ...
        r.raise_for_status()
        data=r.json(); has_more=data.get("has_more",False)
        for slot in data.get("data",[]):
            if slot.get("status")!="AVAILABLE":
                ...
                rows.append({"Date":st.strftime("%m/%d/%Y"),"Start":st.strftime("%I:%M:%S %p").lstrip("0"),
                             "End":en.strftime("%I:%M:%S %p").lstrip("0"),"Resource":slot.get("name","")})
        page+=1; time.sleep(3)
...
def save_csv(rows,path=None):
    if not path:
        ...
        path=os.path.join(os.path.dirname(os.path.abspath(__file__)),f"tjs_sports_{ts}.csv")
...
def main():
    ...
    token=get_token(); rows=fetch(start,end,token)
